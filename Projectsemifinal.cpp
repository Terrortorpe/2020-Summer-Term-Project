...
void Capacitornode::constraint_cons(Matrix& mat, Vector& vec, double t, int& cnt) {
	...
	vec(cnt) = edges[1]->B->Voltage - edges[0]->B->VoltageID;
	++cnt;
}
...
int main() {
	...
	ifstream something("D:/Programming/Recursion project/Tets/netlist.txt");
	string netlist_line;
	...
	for (double time = starttime; time < stoptime; time = time + timestep) {
		...
		Matrix mat = Matrix(cnt_cons, IDcount);
		Vector vec = Vector(cnt_cons);
		...
		mat = Matrix(cnt_td, dIDcount);
		vec = Vector(cnt_td);
		Vector rest = Vector(dIDcount);
